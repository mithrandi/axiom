...
class SystemVersion(item.Item):
    ...
    creation = attributes.timestamp(
        doc="When this system version set was recorded.", allowNone=False
    )
    ...
    def longWindedRepr(self):
        ...
        return '\n\t'.join(
            ...
            + [
                repr(sv)
                for sv in self.store.query(
                    SoftwareVersion, SoftwareVersion.systemVersion == self
                )
            ]
        )
...
class SoftwareVersion(item.Item):
    ...
    systemVersion = attributes.reference(
        doc="The system version this package version was observed in.", allowNone=False
    )

    package = attributes.text(doc="The software package.", allowNone=False)
    version = attributes.text(
        doc="The version string of the software.", allowNone=False
    )
    major = attributes.integer(doc='Major version number.', allowNone=False)
    minor = attributes.integer(doc='Minor version number.', allowNone=False)
    micro = attributes.integer(doc='Micro version number.', allowNone=False)
...
def checkSystemVersion(s, versions=None):
    ...
    mostRecentSystemVersion = s.findFirst(
        SystemVersion, sort=SystemVersion.creation.descending
    )
    mostRecentVersionMap = dict(
        [
            (v.package, v.asVersion())
            for v in s.query(
                SoftwareVersion,
                (SoftwareVersion.systemVersion == mostRecentSystemVersion),
            )
        ]
    )
