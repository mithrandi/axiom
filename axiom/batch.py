...
class BatchProcessingError(item.Item):
    processor = attributes.reference(
        doc="""
    The batch processor which owns this failure.
    """
    )

    listener = attributes.reference(
        doc="""
...
class _ReliableListener(item.Item):
    ...
    backwardMark = attributes.integer(
        doc="""
    ...
    forwardMark = attributes.integer(
        doc="""
    ...
    def __repr__(self):
        return '<ReliableListener %s %r #%r>' % (
            {iaxiom.REMOTE: 'remote', iaxiom.LOCAL: 'local'}[self.style],
            self.listener,
            self.storeID,
        )
    ...
    def _extraWork(self):
        return self.store.query(
            _ReliableTracker, _ReliableTracker.listener == self, limit=2
        )
...
class _BatchProcessorMixin:
    def step(self, style=iaxiom.LOCAL, skip=()):
        ...
        for listener in self.store.query(
            ...
            if not first:
                if VERBOSE:
                    log.msg(
                        "Found more work to do, returning True from %r.step()" % (self,)
                    )
                return True
        ...
        if VERBOSE:
            log.msg(
                "No listeners left with work, returning False from %r.step()" % (self,)
            )
        return False
    ...
    def addReliableListener(self, listener, style=iaxiom.LOCAL):
        ...
        for work in self.store.query(
            self.workUnitType, sort=self.workUnitType.storeID.descending, limit=1
        ):
    ...
    def removeReliableListener(self, listener):
        ...
        self.store.query(
            ...
            attributes.AND(
                ...
                BatchProcessingError.listener == listener,
            ),
        ).deleteFromStore()
    ...
    def getReliableListeners(self):
        ...
        """
        for rellist in self.store.query(
            _ReliableListener, _ReliableListener.processor == self
        ):
    ...
    def getFailedItems(self):
        ...
        """
        for failed in self.store.query(
            BatchProcessingError, BatchProcessingError.processor == self
        ):
...
def upgradeProcessor1to2(oldProcessor):
    ...
    newProcessor = oldProcessor.upgradeVersion(
        oldProcessor.typeName, 1, 2, busyInterval=oldProcessor.busyInterval
    )
...
def processor(forType):
    ...
    try:
        ...
        attrs = {
            ...
            'scheduled': attributes.timestamp(
                doc="""
...
class BatchProcessingControllerService(service.Service):
    ...
    def call(self, itemMethod):
        ...
        return self.batchController.getProcess().addCallback(
            CallItemMethod(
                storepath=item.store.dbdir, storeid=item.storeID, method=method
            ).do
        )
...
class BatchProcessingProtocol(JuiceChild):
    ...
    def command_SUSPEND_PROCESSOR(self, storepath, storeid):
        return (
            self.subStores[storepath.path].suspend(storeid).addCallback(lambda ign: {})
        )
    ...
    def command_RESUME_PROCESSOR(self, storepath, storeid):
        return (
            self.subStores[storepath.path].resume(storeid).addCallback(lambda ign: {})
        )
    ...
    def _pollSubStores(self):
        ...
        try:
            paths = set(
                [
                    p.path
                    for p in self.siteStore.query(substore.SubStore).getColumn(
                        "storepath"
                    )
                ]
            )
    ...
    def step(self):
        while True:
            ...
            while items:
                ...
                if VERBOSE:
                    log.msg(
                        "Stepping processor %r (suspended is %r)"
                        % (item, self.suspended)
                    )
                try:
                    itemHasMore = item.store.transact(
                        item.step, style=self.style, skip=self.suspended
                    )
                except _ProcessingFailure as e:
                    log.msg(
                        "%r failed while processing %r:"
