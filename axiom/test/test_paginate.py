...
class CrossTransactionIteration(TestCase):
    def test_separateTransactions(self):
        ...
        self.assertEquals(s.transact(lambda: (itr.next(), itr.next())), (b2, b3))
    ...
    def test_moreItemsNotMoreWorkSorted(self):
        ...
        """
        s = Store()
        self._checkEfficiency(
            s.query(
                SingleColumnSortHelper, sort=SingleColumnSortHelper.mainColumn.ascending
            )
        )
    ...
    def test_moreThanOneColumnSort(self):
        ...
        self.assertEquals(
            list(
                s.query(
                    MultiColumnSortHelper,
                    sort=[
                        MultiColumnSortHelper.columnOne.ascending,
