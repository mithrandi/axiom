...
class BasicQuery(TestCase):
    def test_rightHandStoreIDComparison(self):
        ...
        self.assertEquals(
            ...
            '(%s.[reftoc] = %s.oid)' % (A.getTableName(s), B.getTableName(s)),
        )
    ...
    def test_leftHandStoreIDComparison(self):
        ...
        self.assertEquals(
            comparison.getQuery(s),
            '(%s.oid = %s.[reftoc])' % (B.getTableName(s), A.getTableName(s)),
        )
        self.assertEquals(comparison.getArgs(s), [])
    ...
    def test_unionComparison(self):
        ...
        """
        s = Store()
        query = ItemQuery(s, A, AND(A.reftoc == B.storeID, B.cref == C.storeID))
        sql, args = query._sqlAndArgs('SELECT', '*')
        self.assertEquals(
            sql,
            'SELECT * FROM %s, %s, %s '
        ...
        def entesten():
            ...
            q = list(
                s.query(
                    B,
                    AND(
                        AND(C.name == u'yes', A.type == u'testc'),
    ...
    def testAttributeQueryDistinct(self):
        ...
        def entesten():
            ...
            self.assertEquals(
                list(s.query(E, sort=E.name.ascending).getColumn("name").distinct()),
                [u"a", u"b"],
            )
    ...
    def test_itemQueryDistinct(self):
        ...
        def entesten():
            ...
            B(store=s, cref=theC)
            B(store=s, cref=theC)
            self.assertEquals(list(s.query(C, B.cref == C.storeID).distinct()), [theC])
    ...
    def test_itemQueryDistinctCount(self):
        ...
        def entesten():
            for n, name in (2, u'it'), (3, u'indefinite nominative'):
                ...
                for i in range(n):
                    B(store=s, cref=theC)
            self.assertEquals(s.query(C, B.cref == C.storeID).distinct().count(), 2)
...
class MultipleQuery(TestCase):
    ...
    def test_basicJoin(self):
        ...
        def entesten():
            ...
            query = s.query((B, C), B.cref == C.storeID, sort=C.name.ascending)
    ...
    def test_count(self):
        ...
        def entesten():
            ...
            query = s.query((B, C), B.cref == C.storeID)
            ...
            for offset in range(totalCombinations):
                for limit in range(totalCombinations + 1):
                    query = s.query(
                        (B, C), B.cref == C.storeID, offset=offset, limit=limit
                    )
    ...
    def test_distinct(self):
        ...
        def entesten():
            ...
            query = s.query(
                (B, C),
                AND(B.cref == C.storeID, A.reftoc == B.storeID),
    ...
    def test_tree(self):
        ...
        def entesten():
            ...
            query = s.query(
                (Gen1, Gen2, Gen3),
                AND(Gen3.cref == Gen2.storeID, Gen2.cref == Gen1.storeID),
            ...
            self.assertEquals(
                tuple(b.name for b in iter(query).next()), (u"Pops", u"Dad", u"Bro")
            )
    ...
    def test_oneTuple(self):
        ...
        def entesten():
            ...
            query = s.query((C,), sort=C.name.ascending)
...
class TableOrder(TestCase):
    ...
    def test_invalidComparison(self):
        ...
        """
        store = Store()
        self.assertRaises(ValueError, store.query, A, B.name == u'value')
    ...
    def test_invalidSort(self):
        ...
        """
        store = Store()
        self.assertRaises(ValueError, store.query, A, sort=B.name.ascending)
    ...
    def test_singleJoin(self):
        ...
        self.assertEqual(
            query.fromClauseParts, [store.getTableName(A), store.getTableName(B)]
        )
    ...
    def test_singleJoinReversed(self):
        ...
        self.assertEqual(
            query.fromClauseParts, [store.getTableName(B), store.getTableName(A)]
        )
    ...
    def test_explicitTableOrder(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                store.getTableName(D),
                store.getTableName(C),
                store.getTableName(B),
...
class AndOrQueries(QueryingTestCase):
    ...
    def testMultipleAndConditions(self):
        ...
        self.assertQuery(
            condition, expectedSQL, ['Narf!', 'Poiuyt!', 'Try to take over the world']
        )
        self.assertEquals(
            self.query(
                D, AND(D.one == 'd1.one', D.two == 'd1.two', D.three == 'd1.three')
            ),
            [self.d1],
        )
    ...
    def testMultipleOrConditions(self):
        ...
        q = self.query(D, OR(D.one == 'd1.one', D.one == 'd2.one', D.one == 'd3.one'))
...
class SetMembershipQuery(QueryingTestCase):
    def test_oneOfValueQueryGeneration(self):
        ...
        self.assertEquals(
            ...
            '%s IN (?, ?, ?)' % (C.name.getColumnName(self.store),),
        )
        self.assertEquals(comparison.getArgs(self.store), values)
    ...
    def test_oneOfColumnQueryGeneration(self):
        ...
        self.assertEquals(
            comparison.getQuery(self.store),
            '%s IN (%s)'
            % (C.name.getColumnName(self.store), A.type.getColumnName(self.store)),
        )
        self.assertEquals(comparison.getArgs(self.store), [])
    ...
    def test_oneOfColumnQueryQueryGenerationWithArguments(self):
        ...
        """
        value = '10'
        subselect = self.store.query(D, AND(D.id == value, D.four == C.name)).getColumn(
            'one'
        )
        ...
        self.assertEquals(map(str, comparison.getArgs(self.store)), [value])
    ...
    def testOneOfWithSet(self):
        ...
        self.assertEquals(
            list(
                s.query(C, C.name.oneOf(set([u'x', u'z', u'a'])), sort=C.name.ascending)
            ),
            [cx, cz],
        )
...
class WildcardQueries(QueryingTestCase):
    ...
    def testOneColumn(self):
        self.assertQuery(
            D.one.like(D.two),
            '(%s LIKE (%s))'
            % (D.one.getColumnName(self.store), D.two.getColumnName(self.store)),
        )
    ...
    def testOneColumnAndStrings(self):
        self.assertQuery(
            ...
            '(%s LIKE (? || %s || ?))'
            % (D.four.getColumnName(self.store), D.id.getColumnName(self.store)),
            [u'%', u'%four'],
        )
    ...
    def testMultipleColumns(self):
        ...
        self.assertEquals(self.query(D, D.one.like(D.two, '%', D.three)), [])
    ...
    def testStartsEndsWith(self):
        ...
        self.assertEquals(self.query(D, D.four.startswith(u'd1')), [self.d1])
        self.assertEquals(self.query(D, D.four.endswith(u'3.four')), [self.d3])

    def testStartsEndsWithText(self):
        self.assertEquals(self.query(D, D.four.startswith(u'd1')), [self.d1])
        self.assertEquals(self.query(D, D.four.endswith(u'2.four')), [self.d2])
...
class UniqueTest(TestCase):
    ...
    def testUniqueNotFoundError(self):
        self.assertRaises(
            errors.ItemNotFound, self.s.findUnique, C, C.name == u'non-existent'
        )

    def testUniqueNotFoundDefault(self):
        bing = object()
        self.assertEquals(
            bing, self.s.findUnique(C, C.name == u'non-existent', default=bing)
        )

    def testUniqueDuplicate(self):
        self.assertRaises(
            errors.DuplicateUniqueItem, self.s.findUnique, C, C.name == u'non-unique'
        )
...
class PlaceholderTestCase(TestCase):
    ...
    def _placeholderAttributeSimilarity(self, kind, sql, args):
        ...
        self.failUnless(IComparison.providedBy(comparison))
        self.assertEquals(
            comparison.getQuery(s), sql % (p.characters.getColumnName(s),)
        )
    ...
    def test_placeholderAttributeDisimilarity(self):
        ...
        operator.
        """
        return self._placeholderAttributeSimilarity(
    ...
    def test_placeholderContainment(self):
        ...
        self.failUnless(IComparison.providedBy(comparison))
        self.assertEquals(
            comparison.getQuery(s), '%s IN (?, ?, ?)' % (p.attr.getColumnName(s),)
        )
    ...
    def test_placeholderAntiContainment(self):
        ...
        self.failUnless(IComparison.providedBy(comparison))
        self.assertEquals(
            comparison.getQuery(s), '%s NOT IN (?, ?, ?)' % (p.attr.getColumnName(s),)
        )
        self.assertEquals(comparison.getArgs(s), value)
    ...
    def test_placeholderContainmentTarget(self):
        ...
        self.assertEquals(
            comparison.getQuery(s),
            '%s IN (%s)'
            % (PlaceholderTestItem.attr.getColumnName(s), p.attr.getColumnName(s)),
        )
        self.assertEquals(comparison.getArgs(s), [])
    ...
    def test_placeholderComparisonTables(self):
        ...
        self.assertEquals(theTable.getTableName(s), PlaceholderTestItem.getTableName(s))
    ...
    def test_placeholderComparison(self):
        ...
        s = Store()
        p = Placeholder(PlaceholderTestItem)
        query = ItemQuery(s, PlaceholderTestItem, PlaceholderTestItem.attr == p.attr)
        sql, args = query._sqlAndArgs('SELECT', '*')
        self.assertEquals(
            sql,
            'SELECT * '
    ...
    def test_multiplePlaceholderComparisons(self):
        ...
        query = ItemQuery(
            ...
            PlaceholderTestItem,
            AND(
                PlaceholderTestItem.attr == p1.attr,
    ...
    def test_sortByPlaceholderAttribute(self):
        ...
        query = ItemQuery(s, p, sort=p.attr.ascending)
        sql, args = query._sqlAndArgs('SELECT', '*')

        expectedSQL = (
            'SELECT * ' 'FROM %s AS placeholder_0 ' 'ORDER BY placeholder_0.[attr] ASC'
        )
