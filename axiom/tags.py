...
class Catalog(Item):
    ...
    def tag(self, obj, tagName, tagger=None):
        ...
        if self.store.findFirst(
            Tag, AND(Tag.object == obj, Tag.name == tagName, Tag.catalog == self)
        ):
            return
    ...
    def tagsOf(self, obj):
        ...
        return self.store.query(
            Tag, AND(Tag.catalog == self, Tag.object == obj)
        ).getColumn("name")

    def objectsIn(self, tagName):
        return self.store.query(
            Tag, AND(Tag.catalog == self, Tag.name == tagName)
        ).getColumn("object")
...
def upgradeCatalog1to2(oldCatalog):
    ...
    did not create.
    """
    newCatalog = oldCatalog.upgradeVersion(
        'tag_catalog', 1, 2, tagCount=oldCatalog.tagCount
    )
